...
'no_cache': False,               #  prevent queries needing SUPER from running
...
def configure(config):
        global OPTIONS, CHK_OPTIONS
        collectd.info('Configuring Stuff')
        for node in config.children:
                if node.key == 'Host':
                        OPTIONS['host'] =  node.values[0]
                elif node.key == 'Port':
                        OPTIONS['port'] =  int(node.values[0])
                elif node.key == 'User':
                        OPTIONS['user'] =  node.values[0]
                elif node.key == 'Password':
                        OPTIONS['password'] =  node.values[0]
                elif node.key == 'NoReplicationClient':
                        OPTIONS['no_replication_client'] =  bool(node.values[0])
                elif node.key == 'NoSuper':
                        OPTIONS['no_super'] =  bool(node.values[0])
                elif node.key == 'Heartbeat':
                        OPTIONS['heartbeat'] =  node.values[0]
                elif node.key == 'CacheDir':
                        OPTIONS['cache_dir'] =  node.values[0]
                elif node.key == 'Debug':
                        OPTIONS['debug'] =  bool(node.values[0])
                elif node.key == 'DebugLog':
                        OPTIONS['debug_log'] =  node.values[0]
                elif node.key == 'innodb':
                        CHK_OPTIONS['innodb'] =  node.values[0]
                elif node.key == 'chk_master':
                        CHK_OPTIONS['master'] =  bool(node.values[0])
                elif node.key == 'chk_slave':
                        CHK_OPTIONS['slave'] =  bool(node.values[0])
                elif node.key == 'chk_procs':
                        CHK_OPTIONS['procs'] =  bool(node.values[0])
                elif node.key == 'chk_get_qrt':
                        CHK_OPTIONS['get_qrt'] =  bool(node.values[0])
                else:
                        collectd.warning('mysql_collectd plugin: Unknown config key: %s.' % node.key)


def ss_get_mysql_stats():
...
def log_debug(val):
    ...
    if not OPTIONS['debug'] and not debug_log:
        return
    try:
...
def read():
    global OPTIONS, GAUGE
    result = ss_get_mysql_stats()
    output = []
    ...
    for stat in result.split():
        var_name, value = stat.split(':')
     	
	if (int(value) < 0): value = 0
	val = collectd.Values(plugin='mysql') # % (plugin_instance,type_instance))
	if (var_name in GAUGE): 
		val.type = 'gauge'
	else:
        	val.type = 'counter'
        val.type_instance = var_name
        val.values = [value]
        val.dispatch()
        log_debug('%s: %s - %s' % (var_name,value,val.type))

        # output.append('mysql.%s.%s %s %d \n' % (OPTIONS['host'], var_name, value, unix_ts))   
